...
> boundaries, and context/state verification. **Prerequisites**:
...
protected routes, browser history. **Avoid when**: Server-side routing (Next.js App Router—see
[nextjs.md](nextjs.md)).
...
when**: Testing error handling in event handlers or async code—error boundaries only catch render
errors.
